...
void Shotgun::Fire()
{
	...
	std::shared_ptr<Bullet> BulletFromAGun1 = std::make_shared<Bullet>(Bullet({CurrentPlayer->transform.Position,glm::vec3(CurrentPlayer->transform.Rotation.x,CurrentPlayer->transform.Rotation.y,CurrentPlayer->transform.Rotation.z + 250),CurrentPlayer->transform.Scale }, Utils::CENTER, CurrentPlayer));
	BulletFromAGun1->Init(*body->GetWorld());
	SceneManager::GetInstance()->GetCurrentScene()->AddEntity(BulletFromAGun1);

	std::shared_ptr<Bullet> BulletFromAGun2 = std::make_shared<Bullet>(Bullet({ CurrentPlayer->transform.Position,glm::vec3(CurrentPlayer->transform.Rotation.x,CurrentPlayer->transform.Rotation.y,CurrentPlayer->transform.Rotation.z - 250),CurrentPlayer->transform.Scale }, Utils::CENTER, CurrentPlayer));
